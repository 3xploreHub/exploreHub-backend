...
module.exports.addServiceComponent = async(req, res) => {
    try {
        ...
        Page.findByIdAndUpdate(
            req.params.id, { $push: { services: data } }, { upsert: true },
            function(err, result) {
                if (err) {
...
module.exports.saveItem = async(req, res) => {
    try {
        ...
        helper.editComponent(Page, { "_id": req.params.parentId, "services._id": req.params.serviceId }, { $push: { "services.$.data": newItem._id } }, res, newItem);
    } catch (error) {
        console.log(error);
        helper.handleError(error, res);
    }
...
module.exports.addComponentImage = (req, res) => {
    ...
    helper.editComponent(Page, { "_id": req.params.parentId, "components._id": req.params.childId }, { $push: { "components.$.data": newImage } }, res, newImage);
}
...
module.exports.deleteComponent = (req, res) => {
    // const Pages = req.params.pageType == 'service' ? servicePage : touristSpotPage;
    helper.deleteItem(Page, { _id: req.params.id }, { 'components': { '_id': req.params.componentId } }, res, req.body.images)
}

module.exports.deleteInputField = (req, res) => {
    // const Pages = req.params.pageType == 'service' ? servicePage : touristSpotPage;
    helper.deleteItem(Page, { _id: req.params.parentId }, { 'bookingInfo': { '_id': req.params.childId } }, res, req.body.images)
}
